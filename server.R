...
shinyServer(function(input, output) {
        ...
        output$dep_delay_plot <- renderPlot({
                g<-ggplot(data=flights2(),aes(x=dep_delay))
                g<-g+geom_histogram()+ggtitle( paste("Departure delays for flights leaving ",input$airport,"on",input$month,"-",input$day))
                g
        ...
        output$arr_delay_plot <- renderPlot({
                g<-ggplot(data=flights2(),aes(x=arr_delay))
                g<-g+geom_histogram()+ggtitle( paste("Arrival delays for flights leaving",input$airport,"on",input$month,"-",input$day))
                g
